...
module ``07: Strings and Conversions`` =
    ...
    let ``02 Getting a substring (Part 1).`` () =
        let a = "bright"
        a.[1..] |> should equal "right"     //start of the index to the end of string
    ...
    let ``07 Make an int conversion fail`` () =
        // we will learn the safe method of doing this when we do Options
        (fun () ->
            let a = "hi" // <-- this value, when converted...             // "17" doesn't work  //can convert h and i into ints
    ...
    *)
    ...
    let ``13 String formatting: %f format specifier`` () =
        ...
        let rounded = sprintf "Multiply by %.1f, then triple" 2.26   //rounds up to the nearest whole 
    ...
    let ``14 String formatting: %A format specifier`` () =
        ...
        let moreResult = sprintf "The %A time-coordinate was used." (8,3,"UTC")  // back lines appear
        moreResult |> should equal "The (8, 3, \"UTC\") time-coordinate was used." // \ ensures that it does'nt the string
   ...
   // objects with F#.
    ...
    let ``17 You can use the "usual" C# string methods from F#`` () =
        ...
        let ``first index of 'P'`` = s.IndexOf('P', 0)
