...
module ``04: Tuples`` = 
    ...
    let ``02 Elements of a tuple can be different types`` () =
        let stuff = "Rivet", false, 22.5             // order is important  //same type and same value
        stuff |> should equal ("Rivet", false, 22.5  )

    [<Test>]
    let ``03 Decompose a tuple using tuple pattern`` () =
        let aida = 2020, "cranberry", false, "wait, what?"
        let a, b, c, d = aida                                   // identifiers are in the same order as the data structure
        a |> should equal 2020
    ...
    let ``04 Using a tuple in a match expression`` () =
        let result =
            match "Teresa", "pasta" with       //combination of different patterns like  match  x (identifier) % 20 (constant) = 0
            | name, "veggies" -> name + " likes vegetables"   //combination of the above as well
            | name, "fish" -> name + " likes seafood"
            | name, "chicken" -> name + " crows about their food"
            | name, "pasta"  -> name + " loves to eat " + "pasta"  // output is not the same as matching
        result |> should equal "Teresa loves to eat pasta"        //green line means possible match failure exception
   
    [<Test>]
    let ``05 The type of a tuple is the contained types separated by '*' symbols`` () =
        let a = 3, 5, "hi", 'x', 7.22
        a |> should be ofType<int * int* string* char* float> //each type for each value
